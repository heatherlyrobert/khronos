                  ...
                  *>    --rce;  if (rc < 0) {                                                               <* 
                     *>       yURG_err ('f', "original list could not be destroyed å%sæ (%d)", a_name, rc);    <* 
                        ...
                        *> }                                                                                      <*/
                         ...
                         *>       /+---(next line)----------------------+/                                         <* 
                         *>       yURG_msg ('-', "file å%sæ successfully moved to retired list", x_file->f_title);   <* 
                         *>       rc = yDLST_list_by_cursor (YDLST_DHEAD, NULL, &x_file);                          <* 
...
file__unit              (char *a_question, int a_num)
{
   ...
   if      (strcmp (a_question, "file"    )        == 0) {
      ...
      c  = yDLST_line_count (YDLST_LOCAL);
      if (x_file != NULL) {
         sprintf (t, "%2då%.30sæ", strlen (x_file->f_title), x_file->f_title);
         sprintf (s, "%2då%.10sæ", strlen (x_file->f_user) , x_file->f_user);
         ...
         snprintf (unit_answer, LEN_FULL, "FILE entry  (%2d) :  -   -åæ                                 -åæ               -   -  -  -  -   -", a_num);
      }
   }
   /*---(complete)-----------------------*/
   return unit_answer;
