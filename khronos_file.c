...
char
FILE_assimilate        (char a_loc, cchar *a_name)
{
   /*---(locals)-----------+-----+-----+-*/
   char        rce         =  -10;
   char        rc          =    0;
   ...
   DEBUG_INPT  yLOG_enter   (__FUNCTION__);
   /*---(parse file)---------------------*/
   if      (a_loc == LOC_CENTRAL)   rc = yEXEC_central    (my.run_as, a_name, &(my.f_user), &(my.f_uid), &(my.f_desc), x_dir);
   else if (a_loc == LOC_LOCAL  )   rc = yEXEC_acceptable (my.run_as, a_name, &(my.f_user), &(my.f_uid), &(my.f_desc), x_dir);
